...
class ReissnerNordstrom:
    """
    Class for defining Reisser-Nordstrom Goemetry Methods
    """

    @u.quantity_input(time=u.s, M=u.kg, Q=u.C)
    def __init__(self, sph_coords, q, M, Q, time):
        self.M = M
        self.Q = Q
        self.q = q
        ...
        time_vel = reissnerNordstrom_utils.reissnerNordstrom_time_velocity(
            pos_vec, vel_vec, self.M, Q
        )
    ...
    def from_spherical(cls, coords, q, M , Q, time=0*u.s):
        """
        Constructor
    ...
    def from_cartesian(cls, coords, q, M, Q, time=0 * u.s):
        """
        Constructor.
        ...
        return cls(sph_coords, q, M, Q, time)

    def f_vec(self, ld, vec):
        vals = np.zeros(shape=(8,), dtype=float)
        chl = reissnerNordstrom_utils.christoffels(
            vec[1], vec[2], self.M.value, self.Q.value
        )
        maxwell = reissnerNordstrom_utils.maxwell_tensor_contravariant(
            vec[1], vec[2], self.Q.value, self.M.value
        )
        metric = reissnerNordstrom_utils.metric(
            vec[1], vec[2], self.M.value, self.Q.value
        )
